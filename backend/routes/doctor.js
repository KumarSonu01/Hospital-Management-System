...
const { calendarDateToUtcMidnight, startOfUtcDay, endOfUtcDay } = require('../utils/dateHelpers');
...
router.post('/schedule-appointment', auth, async (req, res) => {
  try {
    ...
    const { conditionType = 'normal', disease = 'Follow-up' } = req.body;

    const appointmentDate = calendarDateToUtcMidnight(date);
    ...
    const appointment = new Appointment({
      ...
      conditionType,
      disease,
      computingLayer: conditionType === 'critical' ? 'edge' : conditionType === 'moderate' ? 'fog' : 'cloud',
      priorityScore: conditionType === 'critical' ? 1000 : conditionType === 'moderate' ? 400 : 100
    });
